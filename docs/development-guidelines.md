...
feature ブランチの実装範囲は `tasklist.md` の該当セクションが定義する。

#### 原則

1. **tasklist.md が実装の単位を決定する** — `design.md` は全体像の参照用であり、1 ブランチの実装範囲は `tasklist.md` の該当セクションに列挙されたタスクのみとする
2. **実装着手前にスコープを確認する** — ブランチ作成後、最初に `tasklist.md` の該当セクションを精読し、今回のブランチで触るファイル・変更内容を明確にする
3. **後続ブランチの変更を先取りしない** — 設計上関連があっても、後続ブランチで予定されている変更は含めない

#### チェックリスト（実装着手前）

- [ ] `tasklist.md` の該当ブランチセクションを確認した
- [ ] 今回のブランチで変更するファイル・関数を把握した
- [ ] `design.md` の変更のうち、今回のスコープ外のものを識別した

### feature ブランチの完了条件

feature ブランチを PR・マージする前に、以下の全ステップを完了すること。
ビルドが通るだけでなく実機で動作することを確認してから次のブランチに進む。

#### 必須検証ステップ

1. **ユニットテスト** — `go test` で対象パッケージのテストが全パス
2. **TinyGo ビルド** — `tinygo build -target=waveshare-rp2040-zero` が成功
3. **実機書き込み** — ビルド成果物をターゲットボードに書き込む
4. **実機動作確認** — OS にデバイスとして認識され、期待する動作をすること

#### 背景

Phase 1 以前の旧実装では、実機検証を後回しにしてコードベースを拡大した結果、
実機で動作しない状態に陥りリセットを余儀なくされた。
この教訓から、feature ブランチ単位で実機検証を行い、
動作する状態を常に維持する方針とする。
